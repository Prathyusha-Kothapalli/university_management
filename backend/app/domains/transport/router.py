"""
Transport & Fleet Logistics - FastAPI API Endpoints Router
...
from app.domains.transport.schemas import TransportCreate, TransportUpdate, TransportResponse
...
@router.get("/{entity_id}", response_model=TransportResponse)
...
def create_entity(payload: TransportCreate, db: Session = Depends(get_db)):
    service = TransportService(db)
