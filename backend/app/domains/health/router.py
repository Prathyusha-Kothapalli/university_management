"""
Campus Health & Clinic Management - FastAPI API Endpoints Router
...
from app.domains.health.schemas import HealthCreate, HealthUpdate, HealthResponse
...
@router.get("/{entity_id}", response_model=HealthResponse)
...
def create_entity(payload: HealthCreate, db: Session = Depends(get_db)):
    service = HealthService(db)
