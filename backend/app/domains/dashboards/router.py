"""
Executive & Departmental Dashboards - FastAPI API Endpoints Router
...
from app.domains.dashboards.schemas import DashboardsCreate, DashboardsUpdate, DashboardsResponse
...
@router.get("/", response_model=List[DashboardsResponse])
def list_entities(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
    service = DashboardsService(db)
    return service.get_all(skip=skip, limit=limit)

@router.get("/{entity_id}", response_model=DashboardsResponse)
...
@router.post("/", response_model=DashboardsResponse)
def create_entity(payload: DashboardsCreate, db: Session = Depends(get_db)):
    service = DashboardsService(db)
