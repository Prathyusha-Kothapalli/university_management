"""
Finance, Billing & Payroll - FastAPI API Endpoints Router
...
from app.domains.finance.schemas import FinanceCreate, FinanceUpdate, FinanceResponse
...
@router.get("/{entity_id}", response_model=FinanceResponse)
...
def create_entity(payload: FinanceCreate, db: Session = Depends(get_db)):
    service = FinanceService(db)
