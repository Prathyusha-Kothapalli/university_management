"""
Student Life & Hostel Operations - FastAPI API Endpoints Router
...
from app.domains.hostels.schemas import HostelsCreate, HostelsUpdate, HostelsResponse
...
@router.get("/", response_model=List[HostelsResponse])
def list_entities(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
    service = HostelsService(db)
    return service.get_all(skip=skip, limit=limit)

@router.get("/{entity_id}", response_model=HostelsResponse)
...
@router.post("/", response_model=HostelsResponse)
def create_entity(payload: HostelsCreate, db: Session = Depends(get_db)):
    service = HostelsService(db)
